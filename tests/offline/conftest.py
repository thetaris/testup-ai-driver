...
def pytest_sessionfinish(session, exitstatus):
    ...
    results_filename = f"{log_folder}/offline_test_results_{date_str}.txt"
...
@pytest.fixture(scope="session")
def data_file_path():
    return (Path(__file__).parent.parent / 'data' / 'offline')
...
def pytest_configure(config):
    ...
    date_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_filename = f"{log_folder}/offline_test_log_{date_str}.log"
    logging.basicConfig(
