...
class TestmyWebshop_searched_beanie_with_logo:
    ...
    def test_case_1(self, setup):
        logging.info('Starting test case 1: add beanie to cart')
        session_id = 1234
        user_prompt = "add beanie to cart"
        # Locate and read the HTML file
        file_path = Path(__file__).parent.parent/ 'data'/ 'online'/ 'myWebshop_searched_beanie_with_logo.html'
        logging.debug(f"Reading HTML file from: {file_path}")
        with open(file_path, 'r', encoding='utf-8') as file:
            html_content = file.read()
        actions_executed = """"""
        expected_response = "#autoidtestup41"
        ...
        assert actual_response == expected_response, f"Expected: #autoidtestup41, but got: {actual_response}"
