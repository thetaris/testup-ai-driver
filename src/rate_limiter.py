...
class RateLimiter:
    ...
    def check_and_update_limits(self):
        ...
        if elapsed_time >= 60:  # More than a minute has passed, reset window
            self.reset_window()

        if self.request_count < self.max_requests_per_minute and self.token_count <= self.max_tokens_per_minute:
